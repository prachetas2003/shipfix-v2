 ...
 *
 * The `golden-fullstack` fixture mirrors the known-good demo layout (pnpm
 * workspace: Vite web + Express API + Postgres via `pg`). This test runs the
 * full offline pipeline — analyze -> deterministic synthesis -> validation —
 * and pins the exact plan facts ShipFix relies on to deploy it. If any
 * analyzer/planner/validator change breaks the demo repo, this fails in CI
 * long before a live run does. No LLM, no network.
 */
